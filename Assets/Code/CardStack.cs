...
public class CardStack : MonoBehaviour {
	...
	public void Push(Card card){
		...
		if (faceUp){
			iTween.RotateTo(card.getGameObject(), transform.rotation.eulerAngles, 1.0f);
		}
		else {
			iTween.RotateTo(card.getGameObject(), transform.rotation.eulerAngles - new Vector3(180, 0, 0), 1.0f);
		}
